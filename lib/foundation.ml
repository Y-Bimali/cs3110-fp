...
(* let rep_ok f = if suit_of f.spade = Spades && suit_of f.heart = Hearts &&
   suit_of f.club = Clubs && suit_of f.diamond = Diamonds then f else failwith
   "RI" *)
...
let top_cards f = [ f.spade; f.heart; f.club; f.diamond ]

let is_complete f =
  List.fold_left (fun acc x -> acc && rank_of x = King) true (top_cards f)

let set s h c d = { spade = s; heart = h; club = c; diamond = d }
...
let valid_move f c = suit_matcher (fun fc ac -> num_of ac = num_of fc + 1) f c
...
let put f c = if valid_move f c then card_swap f c else raise InvalidMove
